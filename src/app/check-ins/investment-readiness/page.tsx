...
const READINESS_LEVELS = [
  { label: t('Foundation Building'), color: 'var(--brand-danger)', desc: 'Focus on emergency reserves and debt elimination before entering markets.' },
  { label: t('Market Entry'), color: 'var(--brand-gold)', desc: 'You are ready for low-risk index funds. Start small and consistent.' },
  { label: t('Growth Ready'), color: 'var(--brand-primary)', desc: 'Your foundations are solid. You can take on diversified equity exposure.' },
  { label: t('Aggressive Growth'), color: 'var(--brand-success)', desc: 'Peak readiness. Optimize for tax efficiency and tactical alpha.' },
];
...
export default function InvestmentReadiness() {
  ...
  useEffect(() => {
    // 1. Local
    setHistory(storage.ge'investment_readiness_history', []));

    // 2. Server
    storage.fetch('investment_readiness_history'.then(remote => {
      if (remote) {
        setHistory(remote);
        storage.se'investment_readiness_history', remote);
      }
    });
  }, []);
  ...
  const handleSave = () => {
    ...
    setHistory(updated);
    storage.se'investment_readiness_history', updated);
