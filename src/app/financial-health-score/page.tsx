...
const QUESTIONS = [
  ...
  { id: 'budgetTracking', text: t("Do you track your expenses?"), options: [{ label: t("Always (app/spreadsheet)"), score: 10 }, { label: 'Sometimes', score: 5 }, { label: 'Never', score: 0 }], category: t("Financial Planning") },
  { id: 'debtPayments', text: t("Do you make all loan/credit payments on time?"), options: [{ label: 'Always', score: 10 }, { label: 'Sometimes', score: 5 }, { label: t("Often late"), score: 0 }], category: t("Debt & Obligations") },
  ...
  { id: 'impulseSpend', text: t("How often do you impulse spend?"), options: [{ label: 'Rarely', score: 10 }, { label: 'Sometimes', score: 5 }, { label: 'Often', score: 0 }], category: t("Financial Planning") },
  ...
  { id: 'financialLiteracy', text: t("How would you rate your financial knowledge?"), options: [{ label: 'Expert', score: 10 }, { label: t("Good understanding"), score: 7 }, { label: 'Basic', score: 4 }, { label: t("Very limited"), score: 0 }], category: t("Financial Planning") },
  { id: 'knowsCreditScore', text: t("Do you know your credit score?"), options: [{ label: "Yes, and it's good (750+)", score: 10 }, { label: t("Yes, but it needs work"), score: 5 }, { label: 'No', score: 0 }], category: t("Debt & Obligations") },
  ...
  { id: 'taxPlanning', text: t("Do you proactively plan for taxes?"), options: [{ label: t("Yes, maximize deductions"), score: 10 }, { label: 'Somewhat', score: 5 }, { label: t("No planning"), score: 0 }], category: t("Financial Planning") },
];

const CATEGORY_MAX: Record<string, number> = {
  t("Income & Stability"): 20,
  t("Debt & Obligations"): 30,
  t("Savings & Emergency"): 40,
  t("Investment & Growth"): 30,
  t("Financial Planning"): 40,
};
...
const SCORE_LABELS = [
  ...
  { min: 60, max: 75, label: 'Fair', color: 'var(--brand-accent)', emoji: '⚖️', advice: t("You have basics covered. Optimize and accelerate.") },
  { min: 75, max: 90, label: 'Good', color: '#6C5CE7', emoji: '🌟', advice: t("Strong foundation. Time to build wealth aggressively.") },
  { min: 90, max: 101, label: 'Excellent', color: 'var(--brand-success)', emoji: '🏆', advice: t("You are in an elite group. Maintain and mentor others.") },
];
      ...
      <PageHeader 
        ...
        rightSlot={step === totalQuestions ? (
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => { setStep(-1); setAnswers({}); }} className=t("btn btn-secondary btn-icon btn-sm")><RotateCcw size={14} /></button>
          </div>
        ) : null}
      />
      <div style={{ maxWidth: 640, margin: t("0 auto"), padding: 'var(--space-6) var(--space-4) var(--space-16)' }}>
        ...
        {step === -1 && (
          <div style={{ textAlign: 'center', padding: 'var(--space-12) var(--space-4)', animation: t("fadeIn 0.5s ease") }}>
            ...
            <button className=t("btn btn-primary btn-lg") onClick={() => setStep(0)} style={{ minWidth: 220, background: 'linear-gradient(135deg, #F39C12, #f1c40f)', border: 'none' }}>
        ...
        {step >= 0 && step < totalQuestions && (
          <div style={{ marginBottom: 'var(--space-8)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
              <button onClick={() => setStep(step === 0 ? -1 : step - 1)} className=t("btn btn-secondary btn-sm")><ChevronLeft size={14} /> {t('Back')}</button>
        ...
        {step >= 0 && step < totalQuestions && currentQ && (
          <div key={step} style={{ animation: t("slideInRight 0.3s ease both") }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <span className=t("badge badge-gold")>{currentQ.category}</span>
            </div>
            ...
            {currentQ.type === 'slider' && (
              <div className="card" style={{ padding: 'var(--space-8)', textAlign: 'center' }}>
                ...
                <button className=t("btn btn-primary btn-lg") style={{ width: '100%', background: '#F39C12', border: 'none' }}
          ...
          <div style={{ animation: t("fadeIn 0.5s ease") }}>
            <div style={{ textAlign: 'center', marginBottom: 'var(--space-10)' }}>
              <div style={{ position: 'relative', display: 'inline-block', marginBottom: 'var(--space-6)' }}>
                <svg width={200} height={200} viewBox=t("0 0 180 180")>
                  ...
                  <circle
                    ...
                    strokeLinecap="round"
                    transform=t("rotate(-90 90 90)")
                    style={{ transition: t("stroke-dashoffset 1.5s cubic-bezier(0.4,0,0.2,1)"), filter: `drop-shadow(0 0 8px ${scoreInfo.color}44)` }}
                  />
                </svg>
              ...
              <p style={{ color: 'var(--text-secondary)', maxWidth: 400, margin: t("0 auto"), lineHeight: 1.6 }}>{scoreInfo.advice}</p>
            </div>
            ...
            <div style={{ background: '#F39C120A', border: '1px solid #F39C1222', borderRadius: 'var(--radius-xl)', padding: 'var(--space-6)' }}>
              ...
              <div className="stack-6">
                {[
                  { timeframe: t("Immediate Actions"), icon: Zap, actions: score < 40 ? [t("Establish cash reserve immediately"), t("Audit all active debt interest"), t("Consolidate high-cost credit")] : score < 70 ? [t("Automate primary savings transfer"), t("Audit recurring subscriptions"), t("Establish 50/30/20 discipline")] : [t("Optimize tax-loss harvesting"), t("Review insurance leverage"), t("Update legacy beneficiaries")] },
                  { timeframe: t("Strategic Shift (30 Days)"), icon: Shield, actions: score < 40 ? [t("Secure basic term & health cover"), t("Implement zero-based budgeting"), t("Start debt-snowball execution")] : score < 70 ? [t("Reach 3x monthly coverage reserve"), t("Initialize index-fund exposure"), t("Establish dedicated goal vaults")] : [t("Increase voluntary contributions"), t("Review portfolio rebalancing"), t("Analyze asset correlation")] },
                ].map(section => (
