...
export default function InvestmentPlanner() {
  ...
  useEffect(() => {
    // 1. Local
    const list = storage.ge'invest_history' || [];
    setHistory(Array.isArray(list) ? list : []);
    const local = storage.ge'investment_form', null);
    ...
    storage.fetch('invest_history'.then(remote => {
      if (Array.isArray(remote)) {
        setHistory(remote);
        storage.se'invest_history', remote);
      }
    });
    ...
    storage.fetch('investment_form'.then(remote => {
      if (remote && typeof remote === 'object' {
        ...
        storage.se'investment_form', remote);
      }
    });
  }, []);
  ...
  const compute = useCallback(() => {
    ...
    storage.se'invest_history', newHistory);
  ...
  const handleSave = () => {
    storage.se'investment_form', form);
        ...
        {step === 2 && (
          <div style={{ animation: 'fadeInUp 0.5s ease both' }}>
            <div style={{ background: profile.gradient, borderRadius: 'var(--radius-2xl)', padding: 'var(--space-8)', marginBottom: 'var(--space-6)', color: 'white' }}>
               <div style={{ fontSize: 11, fontWeight: 800, textTransform: 'uppercase', opacity: 0.8, letterSpacing: '0.05em' }}>{'Forecasted Wealth ({{period}}Y)', { period: form.period })}</div>
               <div style={{ fontSize: 44, fontWeight: 900, fontFamily: 'var(--font-display)', margin: '8px 0' }}>{fmt.currency(projectedAmount, true)}</div>
               <div className="badge" style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }}>{'{{strategy}} Strategy', { strategy: t(profile.label) })}</div>
            </div>
