...
export default function LoanEMIPlanner() {
  ...
  useEffect(() => {
    ...
    const list = storage.ge'loan_history' || [];
    setHistory(Array.isArray(list) ? list : []);
    const local = storage.ge'loan_emi_form', null);
    ...
    storage.fetch('loan_emi_form'.then(remote => {
      if (remote && typeof remote === 'object' {
        ...
        storage.se'loan_emi_form', remote);
      }
    });
    ...
    storage.fetch('loan_history'.then(remoteHistory => {
      if (Array.isArray(remoteHistory)) {
        ...
        storage.se'loan_history', remoteHistory);
      }
    });
  }, []);
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: 'var(--space-5) var(--space-4) var(--space-16)' }}>
        ...
        {step === -1 && (
          <div style={{ textAlign: 'center', padding: 'var(--space-12) var(--space-4)', animation: 'fadeIn 0.5s ease' }}>
            ...
            <h1 className="display-sm" style={{ marginBottom: 'var(--space-4)' }}>{"Loan & EMI Strategy"}</h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-md)', lineHeight: 1.6, maxWidth: 400, margin: '0 auto var(--space-10)' }}>
              {"Architect your debt profile. Calculate EMIs, interest impact, and optimize your repayment lifecycle with institutional precision."}
            </p>
            ...
            {history.length > 0 && (
              <div style={{ marginTop: 'var(--space-16)', textAlign: 'left' }}>
                <label className="label-caps" style={{ marginBottom: 'var(--space-4)', display: 'block' }}>{"Recent Evaluations"}</label>
        ...
        {step === 0 && (
          <div style={{ maxWidth: 800, animation: 'fadeIn 0.4s ease' }}>
            <div style={{ marginBottom: 'var(--space-10)' }}>
              ...
              <h1 className="display-sm" style={{ color: 'var(--text-primary)', marginBottom: 'var(--space-3)' }}>{"Borrow with Strategic Precision"}</h1>
              <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-lg)', lineHeight: 1.6 }}>{"Architect your debt profile. Calculate EMIs, total interest impact, and optimize your repayment lifecycle."}</p>
            </div>

            <div className="stack-6">
              <div className="card" style={{ padding: 'var(--space-8)' }}>
                <div className="form-group">
                  <label className="form-label" style={{ fontWeight: 700, marginBottom: 'var(--space-4)' }}>{"Select Debt Type"}</label>
              ...
              <div className="grid-2">
                <div className="card" style={{ padding: 'var(--space-6)' }}>
                  <div className="form-group">
                    <label className="form-label" style={{ fontWeight: 700 }}>{"Principal Amount"}</label>
                ...
                <div className="card" style={{ padding: 'var(--space-6)' }}>
                  <div className="form-group">
                    <label className="form-label" style={{ fontWeight: 700 }}>{"Annual Interest (APR)"}</label>
                    <div className="input-group">
              ...
              <div className="grid-2">
                <div className="card" style={{ padding: 'var(--space-6)' }}>
                  <div className="form-group">
                    <label className="form-label" style={{ fontWeight: 700 }}>{"Debt Duration"}: <span style={{ color: 'var(--brand-primary)' }}>{form.tenureYears} {"Years"}</span></label>
                ...
                <div className="card" style={{ padding: 'var(--space-6)' }}>
                  <div className="form-group">
                    <label className="form-label" style={{ fontWeight: 700 }}>{"Processing Overhead (Fixed)"}</label>
                    <div className="input-group">
              ...
              <button className="btn btn-primary btn-lg" disabled={!form.principal || !form.rate} onClick={compute} style={{ alignSelf: 'flex-start', padding: '16px 32px' }}>
                {"Analyze Repayment"} <ArrowRight size={18} />
              </button>
            </div>
          </div>
        )}
          ...
          <div style={{ animation: 'fadeIn 0.4s ease' }}>
            ...
            <div className="responsive-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--space-6)', marginBottom: 'var(--space-10)' }}>
              <div style={{ background: 'var(--bg-glass-light)', border: '1px solid var(--border-brand)', borderRadius: 'var(--radius-2xl)', padding: 'var(--space-8)' }}>
                <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>{"MONTHLY INSTALLMENT"}</div>
                <div className="display-sm" style={{ color: 'var(--brand-primary)' }}>{fmt.currency(result.emi)}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8, fontWeight: 600 }}>{"Duration"}: {form.tenureYears} {"Productive Years"}</div>
              </div>
              <div className="card" style={{ padding: 'var(--space-8)' }}>
                <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>{"TOTAL REPAYMENT"}</div>
                <div className="display-sm" style={{ color: 'var(--text-primary)' }}>{fmt.currency(result.total, true)}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8, fontWeight: 600 }}>{"Principal + Cumulative Interest"}</div>
              </div>
              <div style={{ background: 'rgba(231,76,60,0.03)', border: '1px solid rgba(231,76,60,0.1)', borderRadius: 'var(--radius-2xl)', padding: 'var(--space-8)' }}>
                <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>{"INTEREST OVERHEAD"}</div>
                <div className="display-sm" style={{ color: '#E74C3C' }}>{fmt.currency(result.interest, true)}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8, fontWeight: 600 }}>{"Loss Factor"}: {interestPercent}% {"of Total Capital"}</div>
              </div>
            </div>
            ...
            <div className="card" style={{ padding: 'var(--space-8)', marginBottom: 'var(--space-10)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-8)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <h2 className="heading-sm" style={{ color: 'var(--text-primary)' }}>{"Repayment Lifecycle"}</h2>
                </div>
                <div style={{ display: 'flex', gap: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, color: 'var(--text-muted)' }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--brand-primary)' }} /> {"PRINCIPAL"}
                  </div>
                   <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, color: 'var(--text-muted)' }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#E74C3C' }} /> {"INTEREST"}
                  </div>
                </div>
              </div>
                ...
                <ClientOnly>
                  <ResponsiveContainer width="100%" height="100%" debounce={100}>
                    <AreaChart data={result.amortization}>
                      ...
                      <XAxis dataKey="year" tickFormatter={v => `${"YR"} ${v}`} tick={{ fill: 'var(--text-muted)', fontSize: 10, fontWeight: 700 }} axisLine={false} tickLine={false} />
                      ...
                      <Area type="monotone" dataKey="principal" name={"Principal"} stroke="var(--brand-primary)" fill="url(#gradPrincipal)" strokeWidth={3} />
                      <Area type="monotone" dataKey="interest" name={"Interest"} stroke="#E74C3C" fill="url(#gradInterest)" strokeWidth={3} />
                    </AreaChart>
                  </ResponsiveContainer>
                </ClientOnly>
              </div>
            ...
            <div className="grid-2" style={{ gap: 'var(--space-6)', marginBottom: 'var(--space-10)' }}>
              <div className="card" style={{ padding: 'var(--space-8)', border: '1px solid var(--brand-gold)', background: 'rgba(253,203,110,0.03)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 'var(--space-6)', color: 'var(--text-primary)' }}>
                  ...
                  <h2 className="heading-sm">{"Financial Threshold Guard"}</h2>
                </div>
                <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: 'var(--space-4)' }}>
                  {"For optimal liquidity, your total EMI burden should not exceed 40% of your monthly take-home income."}
                </p>
                <div style={{ background: 'var(--bg-card)', padding: 'var(--space-4)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)' }}>{"MINIMUM INCOME REQ."}</span>
                  <span style={{ fontSize: 'var(--text-base)', fontWeight: 800, color: 'var(--text-primary)' }}>{fmt.currency(result.emi / 0.4)} <span style={{ fontSize: 10 }}>/{"mo"}</span></span>
                </div>
              </div>
              <div className="card" style={{ padding: 'var(--space-8)', border: '1px solid var(--brand-success)', background: 'rgba(0,168,132,0.03)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 'var(--space-6)', color: 'var(--text-primary)' }}>
                  ...
                  <h2 className="heading-sm">{"Optimization Tactics"}</h2>
                </div>
                <div className="stack-3">
                  {[
            ...
            <div className="card" style={{ padding: 'var(--space-8)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 'var(--space-8)' }}>
                ...
                <h2 className="heading-sm" style={{ color: 'var(--text-primary)' }}>{"Amortization Schedule"}</h2>
              </div>
              <div className="table-wrapper">
                <table style={{ borderCollapse: 'separate', borderSpacing: '0 8px' }}>
                  <thead>
                    <tr style={{ background: 'transparent' }}>
                      <th style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: 10, textTransform: 'uppercase', padding: '12px 8px', textAlign: 'left', letterSpacing: '0.05em' }}>{"TIMELINE"}</th>
                      <th style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: 10, textTransform: 'uppercase', padding: '12px 8px', textAlign: 'left', letterSpacing: '0.05em' }}>{"PRINCIPAL APPLIED"}</th>
                      <th style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: 10, textTransform: 'uppercase', padding: '12px 8px', textAlign: 'left', letterSpacing: '0.05em' }}>{"INTEREST LOAD"}</th>
                      <th style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: 10, textTransform: 'uppercase', padding: '12px 8px', textAlign: 'left', letterSpacing: '0.05em' }}>{"RESIDUAL DEBT"}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {result.amortization.map((row: any) => (
                      <tr key={row.year} style={{ background: 'var(--bg-card-hover)' }}>
                        <td style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: 12, borderRadius: 'var(--radius-md) 0 0 var(--radius-md)' }}>{"YEAR"} {row.year}</td>
            ...
            <button className="btn btn-secondary btn-lg" style={{ marginTop: 'var(--space-8)' }} onClick={() => { setStep(0); setResult(null); }}>
              <ChevronRight size={18} style={{ transform: 'rotate(180deg)' }} /> {"Adjust Parameters")}
            </button>
          </div>
        )}
      </div>
