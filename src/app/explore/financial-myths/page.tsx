...
const MYTHS = [
  ...
  { myth: t("Budgeting means giving up everything fun"), reality: t("Effective budgeting includes a dedicated \"wants\" category. It gives you guilt-free permission to enjoy your money."), verdict: 'MYTH', icon: Layout },
...
const VERDICT_CONFIG = {
  MYTH: { color: 'var(--brand-danger)', icon: XCircle, label: t("Busted") },
  PARTIAL: { color: 'var(--brand-gold)', icon: AlertTriangle, label: t("Partial Truth") },
  COMPLEX: { color: 'var(--brand-accent)', icon: HelpCircle, label: t("Nuanced") }
};
              ...
              <div key={i} className="card card-hover" style={{ borderLeft: `6px solid ${config.color}`, padding: 'var(--space-6)' }}>
                ...
                <div style={{ background: '#f8f9fa', padding: '16px', borderRadius: 12, marginBottom: 12 }}>
                   <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', display: 'flex', gap: 8 }}>
                      <X size={16} color="var(--brand-danger)" /> "{item.myth}"
                   </div>
                </div>
                ...
                <div style={{ background: '#e6f4ea', padding: '16px', borderRadius: 12 }}>
                   <div style={{ fontSize: 13, fontWeight: 600, color: '#00A884', lineHeight: 1.6, display: 'flex', gap: 8 }}>
                      <Check size={18} color="#00A884" /> <span><strong>The Reality:</strong> {item.reality}</span>
                   </div>
                </div>
              </div>
            );
