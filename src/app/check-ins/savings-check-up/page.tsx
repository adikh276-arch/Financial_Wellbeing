...
const SCORES = [
  { label: t('CRITICAL'), color: 'var(--brand-danger)', desc: 'Low liquidity and inconsistent habits. Priority: Build $1,000 starter reserve.' },
  { label: t('STABILIZING'), color: 'var(--brand-gold)', desc: 'Foundations are forming but vulnerable. Priority: Reach 3 months coverage.' },
  { label: t('SECURE'), color: 'var(--brand-primary)', desc: 'Robust savings buffer. Priority: Optimize for yield and growth.' },
  { label: t('ELITE'), color: 'var(--brand-success)', desc: 'Exceptional retention. Priority: Focus on tactical wealth catalysts.' },
];
...
export default function SavingsCheckup() {
  ...
  useEffect(() => {
    // 1. Local load
    setHistory(storage.ge'savings_checkup_history', []));

    // 2. Server sync
    storage.fetch('savings_checkup_history'.then(remote => {
      if (remote) {
        setHistory(remote);
        storage.se'savings_checkup_history', remote);
      }
    });
  }, []);
  ...
  const handleSave = () => {
    ...
    setHistory(updated);
    storage.se'savings_checkup_history', updated);
